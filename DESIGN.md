...
2. **Search** – POST each page of `/s/-/dw/data/v99_9/certificate_search` with the bearer token, filtered to `type=private_key`, until every reported result has been collected.
...
The categorization and pagination behavior are exercised by `test/unit/certificateFetcher.test.js` against an in-process `dw.net.HTTPClient` fake.
